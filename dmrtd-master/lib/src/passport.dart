...
class Passport {
  ...
  //Fonction ajoutée pas présente dans le package de base:
  ...
  //Fonction ajoutéé pas presente dans le package de base:
String? extractPersonalNumber(Uint8List data) {
    int i = 0;
    

    while (i < data.length - 2) {
      // Vérifier qu'on a au moins 3 octets à lire
      // Vérifier si on a le tag 5F10 suivi de 12
      if (data[i] == 0x5F && data[i + 1] == 0x10 && data[i + 2] == 0x12) {
       

        i += 3; // Avancer au début de la valeur
        ...
        // Extraire les 18 octets
        Uint8List value = data.sublist(i, i + 18);

        // Convertir en hexadécimal
        String personalNumberHex = value
        ...
        String personalNumberAscii = String.fromCharCodes(value);
     

        return personalNumberAscii; // Ou personalNumberAscii selon ton besoin
      }

      i++; // Avancer d'un octet
    }
  ...
  /// if calling this function prior establishing session with passport.
  /// 
  /// Fonction ajouté pas présente dans le package de base 
  Future<String?> readPersonalNumber() async {
    try {
      ...
      final rawData = await _exec(() => _api.readFileBySFI(EfDG11.SFI));

      if (rawData == null || rawData.isEmpty) {
        
        return Future.error("DG11 est vide ou inaccessible");
      }

      

      // Extraction directe du Personal Number
      String? personalNumber = extractPersonalNumber(rawData);
      ...
      print(" Erreur dans readPersonalNumber : $e");
